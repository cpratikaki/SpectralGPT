...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(
        ...
        self.decoder_pred = nn.Linear(
            decoder_embed_dim,
            self.t_pred_patch_size * patch_size ** 2 * in_chans * 4,
            bias=True,
        )
        print('decoderembeddim',decoder_embed_dim,  'decpredout',self.t_pred_patch_size * patch_size ** 2 * in_chans * 4 )
        # for 4x patches (i.e., super-resolution 2x upsampling)
        # self.decoder_pred = nn.Linear(
        #     decoder_embed_dim,
        #     4 * patch_size**2 * in_chans // (self.t_pred_patch_size**2),  # scale spatial
        #     bias=True
        # )
    ...
    def patchify(self, imgs):
        ...
        return x
    def patchifyhigh(self, imgs):
        """
        imgs: (B, C, H, W)
        returns: (B, L, patch_size**2 * C)
        """
        assert imgs.ndim == 4, f"Expected 4D input [B, C, H, W], got {imgs.shape}"
        ...
        x = imgs.reshape(N, C, h, p, w, p)  # [B, C, h, p, w, p]
        x = x.permute(0, 2, 4, 3, 5, 1)     # [B, h, w, p, p, C]
        x = x.reshape(N, h * w, p * p * C)  # [B, L, patch_dim]
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        print(x.shape, 'projection output')
        # x = x.permute(0, 1,2).reshape(N, 4096, 192)
        x = x.view(N, 1024, 4, 192)       # assuming patch_dim = 4*192
        x = x.view(N, 4096, 192) 
    ...
    def forward_loss_old_default(self, imgs, pred, mask):
        """
        imgs: [N, 3, T, H, W]
    ...
    def forward_loss(self, highres, lowres, pred, mask):
        """
        highres: [B, C, H, W] (ground truth high-resolution image)
        pred: [B, L_pred, patch_dim] (predicted SR patches)
        mask: [B, L_pred] - for masked prediction loss
        """

        # 1. Patchify high-res ground truth
        print('LOWRES', lowres.shape)
        highres = torch.unsqueeze(highres, dim=1)

        _highres = torch.index_select(
            highres,
            2,
            torch.linspace(
                0,
        ...
        device = pred.device  # use the same device as your model output

        # Move highres to the same device as pred
        _highres = _highres.to(device)
        target = self.patchify(_highres)  # shape: [B, L_pred, patch_dim]
        ...
        # 2. Compute per-patch L2 loss
        loss = (pred - target) ** 2
        loss = loss.mean(dim=-1)  # [B, L_pred]
    ...
    def forward(self, lowres, highres, mask_ratio=0.9):
        # print(imgs.shape,'!!!!!!!!!!!!!!')
        latent, mask, ids_restore = self.forward_encoder(lowres, mask_ratio)
        pred = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
        print(pred.shape)
        loss = self.forward_loss(highres, lowres, pred, mask)
